...
def go_play_level():
    ...
    player_lb.configure(text=f"{player_name(active_black, game_type)} playing now ...",
                        foreground="black" if active_black else "white")
    frames["play"].tkraise()


st_level_bt.configure(font=("Arial", 25, "bold"), padx=50, pady=10, bg="blue", foreground="white",
                      command=go_play_level)
...
gameover_lbl = Label(gameover_fr, text="Game over", font=("Arial", 30, "bold"), pady=30, padx=30, foreground="black",
                     bg="blue")
gameover_lbl.pack()
...
scoretitle_lb.pack()
score_lb.pack()
player_lb = Label(play_fr_cont, text="⚫ Player A playing now ....", font=("Arial", 20, "bold"), foreground="black",
                  bg="green")
player_lb.pack()
...
def go_play(type):
    ...
    if game_type != "p":
        ...
        if game_type == "c":
            menu2_level_cont.pack(side="right")
        else:
            menu2_level_cont.pack_forget()
    else:
...
player_name = lambda active_black, game_type: ("⚫ Computer A" if active_black
                                               else "⚪ Computer B") if game_type == "c" else (
    "⚫ Player A" if active_black
    else "⚪ Player B") if game_type == "p" else ("⚫ Player A" if active_black
                                                 else "⚪ Computer A") if game_type[0] == "p" else (
    "⚫ Computer A" if active_black
    else "⚪ Player A")
    ...
    for row in range(board_size):
        for col in range(board_size):
            ...
            if (row == 3 and (col == 3 or col == 4)) or (row == 4 and (col == 3 or col == 4)):
                ...
                coins[row][col] = "gray"
                board[row][col].config(state=["normal"], command=lambda row=row, col=col: handle_move(row, col),
                                       bg="gray")
    if game_type == "p" or game_type == "pc":
...
def eval_func(coins, is_black):
    ...
    cost = cost + ((my_moves - your_moves) / (my_moves + your_moves if (my_moves + your_moves) else 1)) * abs(
        cost) * 0.5
    return cost
    ...
    for row, col in possible_moves(coins, active_black):
        if valid_moves(coins, row, col, active_black) and not restart:
            ...
            if alg == 0:
                ...
                score = alpha_beta(new_coins, not active_black, depth, False, min_eval_coins, max_eval_coins,
                                   active_black)
            elif alg == 2:
                # iterative deepning minimax algorithm with alpha_beta pruning
                while idepth <= depth and ((datetime.timestamp(datetime.now()) - now) <= 5):
                    val = alpha_beta(new_coins, not active_black, idepth, False, min_eval_coins, max_eval_coins,
                                     active_black, True, now)
                    idepth = idepth + 1
                    if val is not None:
                        score = val
            if score > max_score:
...
def AI_move():
    ...
    (row, col) = best_move(0 if level == "easy" or level == "veasy" else 2 if level == "med" else 1,
                           0 if level == "veasy" else 1 if level == "easy"
